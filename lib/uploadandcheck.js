...
export async function uploadandcheck(file, destination) {
    ...
    if (true || Math.random() > 0.5) {
        ...
        if (fileexist) {
            console.log(["检查网盘中存在此文件，跳过上传成功：", file, remotefile].join("\n"));
    ...
    const fileexist = await checkexist(remotefile);
    if (fileexist) {
        console.log(["检查网盘中存在此文件，上传文件成功：", file, remotefile].join("\n"));
        const endtime = Date.now();
        const durtime = (endtime - starttime) / 1000;
        console.log("用时" + durtime + "秒");
        unlink(file, (err) => {
        ...
        console.warn(["检查网盘中不存在此文件，重新上传文件：", file, remotefile].join("\n"));
