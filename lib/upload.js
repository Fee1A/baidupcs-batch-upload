...
import { directfailure, successerror, successmsg, fatalerror, retrymsg, } from "./uploadfile.js";
...
export async function upload(file, destination) {
    ...
    if (fatalerror.some((m) => stdout.includes(m))) {
        throw Object.assign(new Error("exec command failure! baidupcs-go:" +
            "\n" +
            stdout +
            "\n" +
        ...
        const e = Object.assign(new Error("exec command failure! baidupcs-go:" +
            "\n" +
            stdout +
            "\n" +
            stderr), { stdout, stderr });
